...
import { abi as EverlastStaking } from "../abi/EverlastStaking.json";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const bhdContract = new ethers.Contract(addresses[networkID].BHD_ADDRESS as string, ierc20Abi, signer);
    const sbhdContract = new ethers.Contract(addresses[networkID].SBHD_ADDRESS as string, ierc20Abi, signer);
    let approveTx;
...
export const changeStake = createAsyncThunk(
  ...
  async ({ action, value, provider, address, networkID, callback }: IActionValueAsyncThunk, { dispatch }) => {
    ...
    staking = new ethers.Contract(addresses[networkID].STAKING_ADDRESS as string, EverlastStaking, signer);

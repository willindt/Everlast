...
export const PoolDeposit = props => {
  ...
  const poolAllowance = useSelector(state => {
    return state.account.pooling && state.account.pooling.sbhdPool;
  });
  ...
  const setMax = () => {
    const value = parseFloat(sbhdBalance);
    setQuantity(value);
  ...
  useEffect(() => {
    props.setInfoTooltipMessage([
      "Deposit sEVER to win! Once deposited, you will receive a corresponding amount of 33T and be entered to win until your sEVER is withdrawn.",
    ]);
  }, []);
          ...
          <Box display="flex" alignItems="center" flexDirection={`${isMobileScreen ? "column" : "row"}`}>
            ...
            {address && hasAllowance("sbhd") ? (
              <Button
                ...
                fullWidth
              >
                {txnButtonText(pendingTransactions, "pool_deposit", "Deposit sEVER")}
              </Button>
            ) : (
              <Button
                className="pool-deposit-button"
          ...
          {newOdds > 0 && quantity > 0 && (
            <Box padding={1}>
              <Typography variant="body2" style={{ color: "#33BB33" }}>
                Depositing {quantity} sEVER will increase odds of winning to 1 in {newOdds}&nbsp;
              </Typography>
            </Box>
          )}
          {/* NOTE (Appleseed): added this bc I kept losing track of which accounts I had sEVER in during testing */}
            ...
            <div className="data-row">
              ...
              <Typography variant="body1" align="right">
                {isAppLoading ? (
                  ...
                  <>{new Intl.NumberFormat("en-US").format(sbhdBalance)} sEVER</>
                )}
              </Typography>
            </div>
          </div>
